...
from typing import Optional, Iterable, Sequence, List, Tuple, Union
...
class BeadData(object):
    ...
    def L(self, freq=1e6) -> Optional[complex]:
    ...
    def z(self, f=100e6) -> Optional[complex]:
    ...
    def xpoint(self) -> Optional[float]:
    ...
    def max_Z(self) -> Tuple[float, float]:
        z_max = max(self.Z)
        return z_max, self._f[self.Z.index(z_max)]


def code2imp(code: str) -> float:
    """
    Most parts have its value encoded with three decimals numbers
    where the first two numbers are the mantissa and the third the exponent.
    """
    return float(code[:2]) * 10.0 ** float(code[2])


def in_range(value: Numbers, min_max: Sequence[Numbers]) -> bool:
    """
    Is value in range (inclusive).

    :param value: float, int
    :param min_max: tuple(min_, max_)
    :return:
    """
    return min_max[0] <= value <= min_max[1]


def filescan(size, impedance: Sequence[float], inductance=None):
    ...
    for filename in PARTS:
        ...
        if not os.path.exists(filename):
            print(
                "Warning: file does not exist '{}'.\nUncomment in 'PARTS' dictionary to get rid of this message.".format(
                    filename))
            continue
...
def eng_fmt(value, fmt='.3') -> str:
    """ Float formatting which engineers like, 10**(3*n)
...
def plot(bead_data: Iterable[BeadData], frange: Sequence[float] = (1e6, 1e9), r_min: float = 1.0,
         scale: str = 'loglog') -> None:
    """ Plots the bead_data

    @param bead_data List of BeadData objects
    @param frange    (f-min, f-max), diagram's frequency limits
    @param r_min      diagram's minimum resistance
...
def info(bead_data: Iterable[BeadData]) -> None:
    ...
    for bdata in bead_data:
        ...
        print(
            '{:<20}: L = {:>8}H, X=R @ {:>7}Hz, Zmax = {:>6.1f} @ {:>6}Hz, Z = {:>.1f} @1MHz, {:>.1f} @10MHz, {:>.1f} @100MHz'.format(
                bdata.name,
                eng_fmt(bdata.L(1e6)),
